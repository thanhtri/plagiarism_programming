/*
 * People working in large organizations need this sort of statement to be able
 * to include it in their applications, so here it goes:
 *
 * Copyright (c) 2008, Daniel Barreiro (a.k.a. Satyam). All rights reserved.
 * satyam at satyam dot com dot ar (yes, it ends with ar) It is the intention of
 * the author to make this component freely available for use along the YAHOO
 * User Interface Library so it is licensed with a BSD License:
 * http://developer.yahoo.net/yui/license.txt developped along version: 3.0.0PR2
 * of YUI
 */
...
YUI
        .add( 'progressbar',
                function (Y) {
                    var PB = 'progressbar', MASK = 'mask', BAR = 'bar', TL = 'tl', TR = 'tr', BL = 'bl', BR = 'br', ANIM = 'anim',

                    CONTENT_BOX = 'contentBox', MASK_EL = 'maskEl', BAR_EL = 'barEl',

                    getCN = Y.ClassNameManager.getClassName,

                    IMAGE = 'image', C_MASK = getCN(PB, MASK), C_BAR = getCN(
                            PB, BAR), C_TL = getCN(PB, TL), C_TR = getCN(PB, TR), C_BL = getCN(
                            PB, BL), C_BR = getCN(PB, BR), C_ANIM = getCN(PB,
                            ANIM),

                    BAR_MARKUP = '<div class="' + C_BAR + '"></div>', MASK_MARKUP = '<table class="'
                            + C_MASK
                            + '"><tr><td class="'
                            + C_TL
                            + '"></td><td class="'
                    ...
                    L = Y.Lang, isArray = L.isArray, isBoolean = L.isBoolean, isString = L.isString, isNumber = L.isNumber, isNull = L.isNull;
                    ...
                    Y.extend(
                        ...
                        Y.Widget,
                        {
                            ...
                            initializer : function (config) {
                                ...
                                /*
                                 * If animation is loaded, this will trigger for each frame of
                                 * the animation providing partial values.
                                 */
                                this.publish('changingEvent', {
                                    emitFacade : false
                                });
                                /*
                                 * This will fire at the end of the animation or immediately upon
                                 * changing values if animation is not loaded.
                                 */
                                ...
                                /*
                                 * I'm listening to AttributeProvider's own attribute
                                 * change events to adjust the baraccording to the new sizes.
                                 */
                                this.after('valueChange',
                                        this._redraw);
                                this.after('minValueChange',
                                        this.redraw);
                                this.after('maxValueChange',
                                        this.redraw);
                                this.after('widthChange',
                                        this._onWidthChange);
                                this.after('heightChange',
                                        this._onHeightChange);
                                this.after('barColorChange',
                                        this._onBarColorChange);
                                this.after('backColorChange',
                                        this._onBackColorChange);
                                this.after('borderChange',
                            ...
                            renderUI : function () {
                                ...
                                if (isNull(this.get('height'))) {
                                    this.set('height', cbs.getComputedStyle('height'));
                                }
                                this._rendered = true;
                                this._onWidthChange();
                                this._onHeightChange();
                                ...
                                if (Y.Anim) {
                                    ...
                                    this._anim.after('tween', function (ev) {
                                        this.fire('changingEvent', Math.floor(this._tweenFactor * this._anim.get('elapsedTime')
                                                + this._previousValue));
                                    }, this, true);
                            ...
                            /*
                             * The user does not interact with this
                             * control bindUI: function () { },
                             */
                            syncUI : function () {
                                this.get(CONTENT_BOX).setAttribute('tabIndex', 1).setAttribute('role', 'progressbar').setAttribute(
                                        'aria-valuemin', this.get('minValue')).setAttribute('aria-valuemax', this.get('maxValue'))
                                        .setAttribute('aria-valuenow', this.get('value')).setAttribute('aria-valuetext',
                                                this.get('ariaText').replace('|', this.get('value')));
                                this.redraw();
                            },
                            redraw : function () {
                            ...
                            _redraw : function () {
                                ...
                                var pixelValue = (value - this._mn)
                                        * this._barFactor;
                                this.get(CONTENT_BOX).setAttribute('aria-valuenow', value).setAttribute('aria-valuetext',
                                    this.get('ariaText').replace('|', value));
                                ...
                                if (this._anim) {
                                    ...
                                    switch (this.get('direction')) {
                                        ...
                                        break;
                                        case 'rl':
                                            this._anim.set('to', { width : pixelValue, left : this._barSpace - pixelValue });
                                        break;
                                        case 'tb':
                                            this._anim.set('to', { height : pixelValue });
                                        break;
                                        case 'bt':
                                            this._anim.set('to', { height : pixelValue, top : this._barSpace - pixelValue });
                                        break;
                                    }
                                    this._anim.run();
                                } else {
                                    ...
                                    /*
                                     * If the animation utility hasnot been loaded then changing
                                     * the value will alwayscomplete immediately.
                                     */
                                ...
                                switch (this.get('direction')) {
                                    ...
                                    case 'rl':
                                        this._barSpace = parseInt(this.get('width'), 10)
                                                - parseInt(barEl.getComputedStyle('marginLeft'), 10)
                                                - parseInt(barEl.getComputedStyle('marginRight'), 10);
                                    break;
                                    case 'tb':
                                    case 'bt':
                                        this._barSpace = parseInt(this.get('height'), 10)
                                                - parseInt(barEl.getComputedStyle('marginTop'), 10)
                                                - parseInt(barEl.getComputedStyle('marginBottom'), 10);
                                    break;
                                }
